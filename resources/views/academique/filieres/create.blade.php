...
<div class="pt-6 max-w-2xl mx-auto space-y-6">
    ...
    <nav class="flex items-center text-sm text-gray-500 gap-1.5">
        ...
        <span class="text-gray-800 font-semibold">Ajouter une filière</span>
    </nav>

    {{-- Formulaire Card --}}
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" x-data="{ submitting: false }">
        <div class="px-6 py-5 border-b border-gray-100">
            <h3 class="text-base font-bold text-gray-900">Nouvelle filière</h3>
            <p class="text-xs text-gray-400 mt-0.5">Saisissez les informations de la nouvelle filière académique.</p>
        </div>

        <form method="POST" action="{{ route('academique.filieres.store') }}" class="p-6 space-y-5" @submit="submitting = true">
            @csrf
            ...
            <div>
                ...
                <select name="etablissement_id" id="etablissement_id" required
                    ...
                    <option value="{{ $etab->id }}" {{ old('etablissement_id', $defaultEtablissement->id ?? '') == $etab->id ? 'selected' : '' }}>
                        {{ $etab->nom }} ({{ $etab->code }})
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          ...
                          placeholder="Présentation abrégée de la filière, des débouchés, etc.">{{ old('description') }}</textarea>
            ...
            <div>
                <label class="inline-flex items-center gap-2.5 cursor-pointer group">
                    ...
                    <span class="text-xs text-gray-600 font-semibold group-hover:text-gray-800 transition-colors">
                        Activer immédiatement cette filière (visible pour les pré-inscriptions)
                    </span>
                </label>
            </div>
