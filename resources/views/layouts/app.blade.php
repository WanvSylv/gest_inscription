...
<div class="flex min-h-screen">
    ...
    {{-- ═══════ SIDEBAR ═══════ --}}
    <aside class="sidebar hidden lg:flex flex-col fixed top-0 left-0 h-full w-[240px] bg-white border-r border-gray-100 z-30 py-6">
        ...
        <nav class="flex-1 px-3 space-y-0.5 overflow-y-auto">
            ...
            <a href="{{ route('academique.inscriptions.index') }}" class="nav-item {{ request()->routeIs('academique.inscriptions.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('academique.filieres.index') }}" class="nav-item {{ request()->routeIs('academique.filieres.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('comptable.paiements.index') }}" class="nav-item {{ request()->routeIs('comptable.paiements.*') ? 'active' : '' }}">
    ...
    <div class="flex-1 lg:ml-[240px] flex flex-col min-h-screen">
        ...
        {{-- Mobile sidebar --}}
        <div x-show="open" @click="open = false" class="fixed inset-0 bg-black/40 z-40 lg:hidden" x-transition.opacity x-cloak></div>
        <aside x-show="open" class="fixed top-0 left-0 h-full w-[280px] bg-white border-r border-gray-100 z-50 lg:hidden flex flex-col py-6 shadow-xl" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" x-cloak>
            ...
            <div class="px-3 mt-4 pt-4 border-t border-gray-100">
                ...
                <form method="POST" action="{{ route('logout') }}" id="form-logout-mobile" class="mt-0.5">
                    @csrf
                    <button type="button"
                            @click="$dispatch('confirm-modal', {
                                title: 'Déconnexion',
...
<div x-data="confirmModal()" @confirm-modal.window="open($event.detail)" x-cloak>
    <div x-show="show" class="fixed inset-0 z-[999] flex items-center justify-center p-4"
        ...
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="!loading && cancel()"></div>
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10"
            ...
            <div class="flex gap-3">
                <button @click="cancel()" :disabled="loading" class="flex-1 py-2.5 border border-gray-200 text-gray-600 text-sm font-semibold rounded-xl hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    Annuler
                </button>
                <button @click="confirm()" :disabled="loading" class="flex-1 py-2.5 text-white text-sm font-bold rounded-xl transition-colors flex items-center justify-center gap-2 disabled:opacity-85 disabled:cursor-not-allowed" :class="btnClass">
                    ...
                    <span x-text="loading ? 'Traitement...' : confirmText"></span>
                </button>
            </div>
        </div>
    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('confirmModal', () => ({
        show: false, title: '', message: '', confirmText: 'Confirmer', type: 'warning', formId: null, loading: false,
        ...
        get btnClass() { return {'danger':'bg-red-600 hover:bg-red-700 disabled:bg-red-400','warning':'bg-amber-500 hover:bg-amber-600 disabled:bg-amber-400','success':'bg-green-600 hover:bg-green-700 disabled:bg-green-400','info':'bg-sigan-blue hover:bg-blue-700 disabled:bg-blue-400'}[this.type]??'bg-gray-600'; },
        open(d) { this.title=d.title??'Confirmation'; this.message=d.message??'Continuer ?'; this.confirmText=d.confirmText??'Confirmer'; this.type=d.type??'warning'; this.formId=d.formId??null; this.show=true; this.loading=false; },
        confirm() {
            this.loading = true;
            if(this.formId){
        ...
        cancel()  { if(!this.loading) this.show=false; },
    }));
});
</script>
